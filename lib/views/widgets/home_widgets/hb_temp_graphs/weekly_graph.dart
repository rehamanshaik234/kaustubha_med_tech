...
class HeartBeatTemperatureWeeklyGraph extends StatelessWidget {
  const HeartBeatTemperatureWeeklyGraph({super.key});
  ...
  Widget build(BuildContext context) {
    return Consumer<TrackerProvider>(
      builder: (context, provider, child) {
        ...
        List<_ChartData> monthlyData = provider.tracker.healthMonitoring?.weeklyMonitoring
            ?.map((data) => _ChartData(
          data.week ?? "",
          data.pulse ?? 0,
          data.temperature ?? 0,
...
class _ChartData {
  _ChartData(this.week, this.pulse, this.temperature);
