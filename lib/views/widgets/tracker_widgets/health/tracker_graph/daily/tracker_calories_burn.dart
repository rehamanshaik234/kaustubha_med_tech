...
class TrackerCaloriesDailyGraph extends StatefulWidget {
  const TrackerCaloriesDailyGraph({super.key});

  @override
  State<TrackerCaloriesDailyGraph> createState() => _TrackerCaloriesDailyGraphState();
}

class _TrackerCaloriesDailyGraphState extends State<TrackerCaloriesDailyGraph> {
  @override
    ...
    return Consumer<TrackerProvider>(
      builder: (context, provider, child) {
        ...
        List<_ChartData> dailyData = provider.tracker.healthMonitoring?.dailyMonitoring
            ?.map((data) => _ChartData(data.day ?? "", data.caloriesBurned ?? 1))
            .toList() ??
            [];
        // Calculate min, max, and average values
        final pulseValues = dailyData.map((data) => data.calories).toList();
              ...
              child: SfCartesianChart(
                ...
                series: <CartesianSeries>[
                  ...
                  SplineAreaSeries<_ChartData, String>(
                    ...
                    yValueMapper: (_ChartData data, _) => data.calories, // Y-axis values (pulse)
