...
class _CommunityChatScreenState extends State<CommunityChatScreen> {
            ...
            SizedBox(
              child: ListView.separated(
                separatorBuilder: (context, i) {
                  ...
                  ) : user?.role==Constants.patientRole && i==newMessageIndex? Padding(
                    padding: const EdgeInsets.symmetric(vertical: 8.0),
                    child: Column(
                      children: [
                        Row(
  ...
  void markAsSeen(MessageModel message)async{
    print(message.seenBy.toString());
      UserInfo? user=await LocalDB.getUserInfo();
      socket?.emit('markAsSeen',{
        "userId": user?.id,
        "conversationId": message.conversationId,
        "lastSeenMessageId": message.id,
        'roomId':'community_${message.conversationId}'
      });
  }
  ...
  bool checkIsSeenMessage(MessageModel? message){
      if (user?.role==Constants.patientRole) {
        ...
        isSeen= message?.seenBy?.isNotEmpty==true && message!.seenBy!.length>=2;
        return isSeen;
      }
    }
