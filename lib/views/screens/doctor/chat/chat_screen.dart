...
class _DoctorChatScreenState extends State<DoctorChatScreen> {
  ...
  void initializeSocket(String userId,String clientId) {
    ...
    userSocket!.on('receivedMessage', (data) {
      if(data!=null && mounted){
        ...
        if(newMessageIndex!=null){
          newMessageIndex=newMessageIndex!+1;
        }
        markAsSeen(MessageModel.fromJson(data));
        scrollToBottom();
        setState(() {});
      }
    });
    ...
    userSocket!.on('messagesSeen', (data) {
      if(data!=null && data['updatedMessages']!=null && mounted){
        log("messagesSeen"+data.toString());
        messages.clear();
        final _messages = data['updatedMessages'] as List;
    ...
    userSocket!.on('previousMessages', (data) {
      if (data != null && messages.isEmpty && mounted) {
        ...
        loader = false;
        markAsSeen(MessageModel.fromJson(_messages.last));
        newMessageIndex=getNewMessageIndex(messages);
        setState(() {});
        scrollToBottom();
      }
    });
  ...
  @override
  void dispose() {
    userSocket?.off('joinRoom');
    userSocket?.off('receivedMessage');
    userSocket?.off('conversationId');
    userSocket?.off('previousMessages');
    userSocket?.off('error');
    userSocket?.off('messagesSeen');
    super.dispose();
  }
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(onPressed: (){
             Navigator.of(context).pushNamed(RoutesName.videoCall,arguments: {"clientId": (user?.role==Constants.patientRole?contactInfo.doctorId : contactInfo.userId) ?? ""});
            ...
            SizedBox(
              child: ListView.separated(
                separatorBuilder: (context, i) {
                  ...
                  ): messages[i].senderId == (isPatient? contactInfo.doctorId:contactInfo.userId) && i==newMessageIndex? Padding(
                    padding: const EdgeInsets.symmetric(vertical: 8.0),
                    child: Column(
                      children: [
                        Row(
  ...
  Widget textCard(String message, bool isSender, String date,{EdgeInsets? margin,MessageModel? messageModel}) {
    return Align(
                  ...
                  children: [
                    ...
                    Visibility(
                        ...
                        child: Container(
                            ...
                            child:checkIsSeenMessage(messageModel)? Icon(Icons.done_all,color: Colors.white,size: 16.sp,):Icon(Icons.check,color: Colors.grey,size: 16.sp,)
                        ))
                  ],
                ),
              ],
  ...
  Widget fileView(String filePath,bool isSender,String date,{EdgeInsets? margin,File? localFile,MessageModel? messageModel}) {
    return Align(
      alignment: isSender ? Alignment.topRight : Alignment.topLeft,
      child: InkWell(
        onTap: (){
  ...
  void markAsSeen(MessageModel message)async{
    if(message.seenBy!=null){
      UserInfo? user=await LocalDB.getUserInfo();
      userSocket?.emit('markAsSeen',{
        "userId": user?.id,
  ...
  bool checkIsSeenMessage(MessageModel? message){
    if(message?.seenBy!=null){
      if(user?.role==Constants.patientRole && message?.senderId!=user?.id){
        bool isSeen=false;
        message?.seenBy?.forEach((seen){
          if(seen.userId==contactInfo.userId){
            isSeen=true;
          }
        });
        return isSeen;
        ...
        bool isSeen=false;
        message?.seenBy?.forEach((seen){
          if(seen.userId==contactInfo.doctorId){
            isSeen=true;
          }
        });
        return isSeen;
  ...
  int? getNewMessageIndex(List<MessageModel> messages){
    int? index;
    for (int i = 0; i < messages.length; i++) {
      var message = messages[i];
      if (message.seenBy != null && !checkIsSeenMessage(message)){
          index=i;
      }else{
        if(index!=null){
          break;
