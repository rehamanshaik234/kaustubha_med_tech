...
import 'package:kaustubha_medtech/views/alerts/compression_dialog.dart';
...
import 'package:kaustubha_medtech/views/widgets/doctor_enrollment/countries_dropdown.dart';
...
class _EditProfileScreenState extends State<EditProfileScreen> {
  ...
  void initState() {
    ...
    age=TextEditingController(text: provider.user.age);
              ...
              child: SingleChildScrollView(
                child: Column(
                  children: [
                      ...
                      child: Stack(
                        children: [
                          ClipRRect(
                              ...
                              child:imageLoader?SizedBox(height: 150.sp,width:150.sp,child: Padding(
                                padding: EdgeInsets.all(50.sp),
                                child: CircularProgressIndicator(color: Colors.black,strokeWidth: 2.sp,),
                              ),):Image(image: profile!=null? FileImage(profile?? File('')):provider.user.image!=null?NetworkImage(provider.user.image):AssetImage(AssetUrls.profile),fit: BoxFit.cover,height: 150.h,width: 150.w,)),
                          Positioned(
                              bottom: 12.h,
                              right: 12.w,
                              child: InkWell(
                    ...
                    SizedBox(height: 8.h,),
                    PhoneNumberTextField(hintText: "Enter Number", initialValue: phone.text,
                      onChange: (text){
                        String? number=text?.completeNumber;
                        if(number!=null) {
                        ...
                        child: Column(
                          children: [
                            ...
                            Row(
                              mainAxisAlignment: MainAxisAlignment.start,
                              children: [
                                Text('Age',style: GoogleFonts.dmSans(fontSize: 16.sp,fontWeight: FontWeight.w500),),
                              ],
                            ),
                            SizedBox(height: 8.h,),
                            CustomTextField(hintText: "Enter Age", textEditingController:age,
                              includeSpacing: true,
                              inputType: TextInputType.number,
                              border: OutlineInputBorder(
                                borderRadius: BorderRadius.circular(8.sp),
                          ...
                          mainAxisAlignment: MainAxisAlignment.start,
                          children: [
                            Text('Region',style: GoogleFonts.dmSans(fontSize: 16.sp,fontWeight: FontWeight.w500),),
                          ],
                        ),
                            SizedBox(height: 8.h,),
                            CustomCountriesDropDown(
                          countryValue: country,cityValue: city,stateValue: state,
                          onCitySelect: (city){
                            if(city!=null){
                              this.city=city;
                              setState(() {});
                    ...
                    SizedBox(height: 24.h,),
                    CustomButton(onPressed: ()=>editProfile(provider), title: "Update",loader: provider.loader|| provider.otpLoader,),
  ...
  void editProfile(UserProvider provider){
    if(validateFields()) {
      Map<String, dynamic> params = {
       ...
       if(age.text.isNotEmpty) UserInfo.userAgeKey: age.text,
       if(gender!=null && gender?.isNotEmpty==true) UserInfo.userGenderKey: gender,
       if(country!=null && country?.isNotEmpty==true) UserInfo.userCountryKey: country,
       if(state?.isNotEmpty==true) UserInfo.userStateKey: state,
       if(city?.isNotEmpty==true) UserInfo.userCityKey: city,
      };
  ...
  void onUpdateProfileResponse(ResponseMessage message)async{
    if(message.success!=null){
      ...
      CustomPopUp.showSnackBar(context, message.error ?? 'Something went wrong', Colors.redAccent);
    }
  }
  ...
  bool validateFields(){
    ...
    if(provider.user.email!=null && provider.user.email!.isNotEmpty || email.text.isNotEmpty){
      return validateName()  && validateNumber() && validateEmail() && provider.user.email==null && provider.user.password==null? validatePasswords():true;
    }
