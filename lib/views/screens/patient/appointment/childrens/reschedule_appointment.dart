...
class _RescheduleAppointmentState extends State<RescheduleAppointment> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
          ...
          builder: (context,provider,child) {
            if(error!=null){
              return Center(child: Column(
                ...
                children: [
                  Icon(Icons.timer_off_outlined,color: Colors.orange,size: 24.sp,),
                  Text(error.toString(),style: GoogleFonts.dmSans(fontWeight: FontWeight.w500,fontSize: 16.sp),textAlign: TextAlign.center,),
                ],
              ),);
            }
            return Padding(
      ...
      bottomSheet: error==null? Consumer<PatientAppointmentProvider>(
  ...
  void getDates(){
    ...
    if(data!=null){
      ...
      if(_selectedDate!.isBefore(DateTime.now())){
        _selectedDate=DateTime.now();
        // error="This Meeting Has Expired\nYou can only re-schedule the appointment before 1 day";
      }
    }
    setState(() {});
    getAvailableSlots();
