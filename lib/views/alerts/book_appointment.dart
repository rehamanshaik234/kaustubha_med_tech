...
class AppointmentAlert{
   ...
   return await showDialog<bool>(
        ...
        builder: (context){
          return AlertDialog(
           ...
           content: Text('Your appointment with $doctorName is confirmed on ${Constants.convertDateForEnrollment(date)}, at $time.',
  ...
  static void showRescheduleAppointment(BuildContext context,String id,String doctorId,Function onCancel,Function onReschedule){
    showDialog(context: context,
        builder: (context){
          return Consumer2<PatientAppointmentProvider,DoctorAppointmentProvider>(
              builder: (context,provider,doctorProvider,_) {
                return AlertDialog(
                  ...
                  title: Row(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      Text(provider.loader || doctorProvider.loader ? 'Rescheduling..':"Cancel?",style: GoogleFonts.inter(color: AppColors.primaryColor,fontSize: 20.sp,fontWeight: FontWeight.w600),),
                    ],
                  ),
                  content:provider.loader || doctorProvider.loader?SizedBox(height: 30.h,width: 30.w, child: Center(child: CircularProgressIndicator.adaptive(backgroundColor: Colors.black,strokeWidth: 2.w,),)):Text('You can Re-Schedule the Appointment, before Canceling',
                    style: GoogleFonts.inter(color: Colors.black54,fontSize: 14.sp,fontWeight: FontWeight.w600),textAlign: TextAlign.center,),
                  actions: [
                    Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        CustomButton(onPressed: ()async{
                          Navigator.pop(context);
                          onCancel();
                        }, title: "No, Cancel",borderRadius: BorderRadius.circular(20.sp),height: 30.h,
                          padding: 0,width: 110.w,bgColor: AppColors.buttonSecondaryBgColor,textColor: AppColors.primaryColor,textSize: 14.sp,),

                        CustomButton(onPressed: ()async{
                          await provider.getDoctorDetails(id, (r){});
                          Navigator.pop(context);
                          onReschedule();
                        }, title: "Re-Schedule",borderRadius: BorderRadius.circular(20.sp)
                          ,height: 30.h,padding: 0,width: 110.w,textSize: 14.sp),
                      ],
                    )
                  ],
                );
