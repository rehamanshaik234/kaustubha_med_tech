...
class DoctorAppointmentProvider extends ChangeNotifier{
  DoctorAppointmentRepo appointmentRepo = DoctorAppointmentRepo();
  bool _loader=false;
  ...
  void setFilter({  String? country, String? experienceYears, String? specialization,
    String? state, String? city, String? qualification,bool? resetSpecialization,bool? resetCity,bool? resetExp,bool? resetQualification,bool? resetCountry}){
    _filterModel= _filterModel.copyWith(country: country,experienceYears: experienceYears,
        ...
        resetSpecialisation: resetSpecialization,resetCity:resetCity,resetExp: resetExp,resetQualification:resetQualification,resetCountry: resetCountry);
    notifyListeners();
  }
  ...
  Future<void> getPendingAppointments(Function(ResponseMessage message) onResponse)async{
    _loader=true;
    _error='';
    notifyListeners();
    ApiResponse apiResponse = await appointmentRepo.getPendingAppointments();
  ...
  Future<void> cancelAppointment(Map<String,dynamic> data, Function(ResponseMessage message) onResponse)async{
    ...
    if( apiResponse.response != null && apiResponse.response?.statusCode == 200 ){
      ...
      final response = ResponseMessage.fromJson(map);
      await getUpcomingAppointments((r){});
      await getPendingAppointments((r){});
      onResponse(response);
      _cancelLoader=false;
      notifyListeners();
    }else{
  ...
  Future<void> confirmAppointment(Map<String,dynamic> data, Function(ResponseMessage message) onResponse)async{
    _confirmLoader=true;
    _error='';
    notifyListeners();
    print(data);
    ApiResponse apiResponse = await appointmentRepo.confirmAppointment(data);
    print("apiResponse.Data.success ${apiResponse.response?.data.toString()}");
    if( apiResponse.response != null && apiResponse.response?.statusCode == 200 ){
      Map map = apiResponse.response!.data ?? {};
      final response = ResponseMessage.fromJson(map);
